...
const deriveRootDirFromStateDir = (stateDir) => {
  const normalized = path.resolve(String(stateDir || "").trim());
...
const deriveRootDirFromConfigPath = (configPath) => {
  const normalized = path.resolve(String(configPath || "").trim());
  if (!normalized) return "";
  if (path.basename(normalized) !== "openclaw.json") return "";
  return deriveRootDirFromStateDir(path.dirname(normalized));
};
...
const resolveRootDir = () =>
  ...
  deriveRootDirFromStateDir(process.env.OPENCLAW_STATE_DIR) ||
  deriveRootDirFromConfigPath(process.env.OPENCLAW_CONFIG_PATH) ||
  kFallbackRootDir;
...
module.exports.deriveRootDirFromConfigPath = deriveRootDirFromConfigPath;
module.exports.deriveRootDirFromStateDir = deriveRootDirFromStateDir;
module.exports.resolveRootDir = resolveRootDir;
