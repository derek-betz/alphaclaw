...
const normalizeManagedUsageTrackerPaths = ({ cfg, fs }) => {
...
const applyFreshOnboardingChannels = ({ cfg, varMap, fs }) => {
  ...
  normalizeManagedUsageTrackerPaths({ cfg, fs });
};
...
const writeSanitizedOpenclawConfig = ({ fs, openclawDir, varMap }) => {
  ...
  applyFreshOnboardingChannels({ cfg, varMap, fs });
