  ...
  const runRepair = async ({ source, correlationId, force = false }) => {
    ...
    try {
      ...
      if (ok) {
        ...
        try {
          recoveryResult =
            typeof recoverGateway === "function"
              ? await recoverGateway()
              : { ok: false, skipped: true, reason: "recover_gateway_unavailable" };
          if (recoveryResult?.ok) {
            ...
            logEvent(
              ...
              "repair",
              "failed",
              recoveryResult || { reason: "gateway recovery returned no result" },
              correlationId,
            );
          }
        } catch (err) {
