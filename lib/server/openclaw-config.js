...
const normalizeUsageTrackerPluginPaths = ({
  ...
  for (const currentPath of inputPaths) {
    ...
    if (!firstCandidatePath) firstCandidatePath = normalizedPath;
  ...
  if (!selectedPath && firstCandidatePath) {
    selectedPath = firstCandidatePath;
    nextPaths.push(firstCandidatePath);
  } else if (!selectedPath && fallbackPath) {
